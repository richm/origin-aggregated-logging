...
class Fluent::ElasticsearchOutput < Fluent::ObjectBufferedOutput
  ...
  config_param :log_stats, :bool, :default => false
  ...
  def write_objects(tag, chunk)
    ...
    if @log_stats
      log.error("sent message tag #{tag} at #{startts} took #{Time.now.to_f - startts} bytesize #{bulk_message.size} nrecords #{nrecords}")
    end
    bulk_message.clear
  end
