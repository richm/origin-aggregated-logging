...
create_forwarding_fluentd() {
 ...
 oc create configmap logging-forward-fluentd \
    --from-file=fluent.conf=../templates/mux-fluent.conf
 ...
 # create forwarding daemonset
  ...
  oc patch configmap/logging-forward-fluentd --type=json --patch '[{ "op": "replace", "path": "/data/filter-pre-viaq-mux-transform.conf", "value": "@include ../openshift/filter-viaq-mux-transform.conf"}]'

  oc patch configmap/logging-forward-fluentd --type=json --patch '[{ "op": "replace", "path": "/data/filter-retag-mux.conf", "value": "\
<match kubernetes.**.mux>\n\
  @type rewrite_tag_filter\n\
  rewriterule1 CONTAINER_NAME ^k8s_ forwarded.mux\n\
</match>"}]'
...
ffpod=$(oc get pods -l component=forward-fluentd -o name)
ffpod=`echo $ffpod | awk -F'/' '{print $(NF)}'`

if [ "$ffpod" != "" ]; then
echo $ffpod
oc logs $ffpod
oc exec $ffpod -- cat /etc/fluent/configs.d/user/filter-retag-mux.conf
fi

echo "logging-fluentd configmap"
oc get configmap/logging-fluentd -o yaml 
echo "logging-forward-fluentd configmap"
oc get configmap/logging-forward-fluentd -o yaml 

echo DEBUG PRINT ENDS ---------------------------------------------------
