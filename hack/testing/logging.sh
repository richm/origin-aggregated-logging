...
if [ $USE_JOURNAL = false ] ; then
    ...
    if grep -- '--log-driver=journald' /etc/sysconfig/docker > /dev/null 2>&1 ; then
    ...
    if grep -- '--log-driver=json-file' /etc/sysconfig/docker > /dev/null 2>&1 ; then
        sudo sed -i.bak 's/--log-driver=json-file/--log-driver=journald/' /etc/sysconfig/docker
        sudo systemctl restart docker
    elif grep '^OPTIONS=' /etc/sysconfig/docker > /dev/null 2>&1 ; then
...
function wait_for_app() {
  ...
  wait_for_command '[[ "$(curl -s -X POST http://${FRONTEND_IP}:5432/keys/foo -d value=1337)" = "Key created" ]]' 1000 "curl -s -X POST http://${FRONTEND_IP}:5432/keys/foo -d value=1337"
